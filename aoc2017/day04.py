...
def part1(game_input):
	orphans = functools.reduce(
		lambda acc, row: acc + (0 if has_duplicates(row) else 1),
		game_input,
		0
	)
...
def part2(game_input):
	def has_anagrams(it):
		return has_duplicates(map(lambda x: ''.join(sorted(x)), it))

	orphans = functools.reduce(
		lambda acc, row: acc + (0 if has_anagrams(row) else 1),
		game_input,
		0
	)
...
def main():
	sanitized_input = [list(row.split()) for row in GAME_INPUT.strip().split('\n')]
