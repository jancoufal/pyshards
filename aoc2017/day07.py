...
class Disc(object):
	def __init__(self, name, weight, children=None):
		...
		self._children_discs = None

	def __str__(self):
		return '%s (%d, %d) -> %s' % (self._name, self._weight, self.weight_with_sublevel, str(self._children))
	...
	def set_children_discs(self, discs):
		self._children_discs = discs

	@classmethod
	def parse_from_line(cls, line):
		splat = line.split('->')
		name_and_weight = splat[0].split('(')
		children = None
		if len(splat) > 1:
			children = [ch.strip() for ch in splat[1].strip().split(',')]

		return cls(
			name_and_weight[0].strip(),
			int(''.join(filter(lambda ch: ch in string.digits, name_and_weight[1]))),
			children
		)
	...
	@property
	def children_discs(self):
		return self._children_discs

	@property
	def weight_with_sublevel(self):
		return self._weight + sum([ch_disk.weight_with_sublevel for ch_disk in self._children_discs])


def get_root_name(game_input):
	all_parents = set()
	all_children = set()
	for d in game_input:
		all_parents.add(d.name)
		all_children |= d.children

	root = all_parents - all_children

	if len(root) != 1:
		raise RuntimeError('Root was not found or maybe too many were found.')

	return root.pop()


def build_tree(node_name, flat_map, root_name=None, level=0):

	node = flat_map[node_name]
	node.set_parent_name(root_name)
	node.set_level(level)
	node.set_children_discs(set(flat_map[ch] for ch in node.children))

	for ch in flat_map[node_name].children:
		build_tree(ch, flat_map, node_name, level + 1)


def part1(game_input):
	root = get_root_name(game_input)
	print('root item =', root)


def part2(game_input):
	def search_disbalance(_root_name, _flat_map, _max_level, _level=0):
		node = _flat_map[_root_name]

		weights = list()
		for ch in node.children:
			weights.append(search_disbalance(ch, _flat_map, _max_level, _level + 1))

		if _level < _max_level and len(weights) > 0 and min(weights) != max(weights):
			for node_ch in node.children_discs:
				print((_level + 1) * '  ' + str(node_ch))
			print(_level * '  ' + str(node))
			print()

		return node.weight_with_sublevel

	root = get_root_name(game_input)
	flat_map = {disc.name: disc for disc in game_input}

	build_tree(root, flat_map)

	max_level = max([disc.level for disc in game_input])

	print('max_level =', max_level)

	search_disbalance(root, flat_map, max_level)


def main():
	sanitized_input = [Disc.parse_from_line(row) for row in GAME_INPUT.strip().split('\n')]

	# for d in sanitized_input:
	# 	print(d)
