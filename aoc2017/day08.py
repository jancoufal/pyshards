...
class Instruction(object):
	def __init__(self, reg, func, val, test_reg, test_op, test_val):
		self.register = reg
		self.func = func
		self.value = int(val)
		self.test_register = test_reg
		self.test_operator = test_op
		self.test_value = int(test_val)

	@classmethod
	def parse_from_line(cls, line):
		splat = line.strip().split()
		return cls(*filter(lambda tok: tok != 'if', splat))

	def get_operation_tuple(self):
		return self.register, self.func, self.value

	def get_predicate_tuple(self):
		return self.test_register, self.test_operator, self.test_value
...
class Registers(object):
	...
	def __init__(self, registers):
		self._registers = {register: 0 for register in registers}
		self._highest_value_ever = None

	@classmethod
	def build_from_instructions(cls, instructions):
		return cls(set(instruction.register for instruction in instructions))

	def check_predicate(self, register, operator, value):
		return Registers.predicate_map[operator](self._registers[register], value)

	def do_operation(self, register, func, value):
		self._registers[register] = Registers.function_map[func](self._registers[register], value)
		return self._registers[register]

	def get_highest_register_value(self):
		return max(self._registers.values())


def part1(game_input):

	registers = Registers.build_from_instructions(game_input)

	for instruction in game_input:
		if registers.check_predicate(*instruction.get_predicate_tuple()):
			registers.do_operation(*instruction.get_operation_tuple())

	print('highest register value =', registers.get_highest_register_value())


def part2(game_input):

	registers = Registers.build_from_instructions(game_input)
	reg_highest_value = None

	for instruction in game_input:
		if registers.check_predicate(*instruction.get_predicate_tuple()):
			reg_val = registers.do_operation(*instruction.get_operation_tuple())

			if reg_highest_value is None or reg_highest_value < reg_val:
				reg_highest_value = reg_val

	print('highest register value ever reached =', reg_highest_value)


def main():
	sanitized_input = [Instruction.parse_from_line(row) for row in GAME_INPUT.strip().split('\n')]
